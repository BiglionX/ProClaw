...
var CTASection_1 = require("../components/shared/CTASection");
...
var UseCasesPage = function () {
    var _a = (0, react_1.useState)('all'), activeFilter = _a[0], setActiveFilter = _a[1];
    (0, react_1.useEffect)(function () {
        (0, pluginService_1.getPublishedPlugins)().then(function () { });
    }, []);
    // 从默认场景合并生成筛选标签
    var industryFilters = __spreadArray([
        { key: 'all', label: '全部场景' }
    ], defaultUseCases
        .filter(function (uc, i, arr) { return arr.findIndex(function (u) { return u.industry === uc.industry; }) === i; })
        .map(function (uc) { return ({
        key: uc.industry,
        label: uc.industryLabel,
        emoji: uc.iconEmoji,
    }); }), true);
    var filteredCases = defaultUseCases.filter(function (uc) {
        return activeFilter === 'all' ? true : uc.industry === activeFilter;
    });
    return (<div className="min-h-screen bg-gray-50 flex flex-col">
      <RouteSEO_1.default routeKey="useCases"/>
      <Navbar_1.default />

      <PageHeader_1.default title="选对你的场景，看看 ProClaw 能帮什么忙" description="4 个行业场景，看看哪个说的是你"/>

      {/* Industry Filter Tabs */}
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-center gap-2 flex-wrap">
            {industryFilters.map(function (tab) { return (<button key={tab.key} onClick={function () { return setActiveFilter(tab.key); }} className={"px-5 py-2 rounded-lg text-sm font-medium transition-all ".concat(activeFilter === tab.key
                ? 'bg-black text-white shadow-md'
                : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}>
                {tab.emoji ? "".concat(tab.emoji, " ").concat(tab.label) : tab.label}
              </button>); })}
          </div>
        </div>
      </div>
      ...
      <div className="flex-grow py-12">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-8">
            {filteredCases.map(function (uc) { return (<div key={uc.id} className="bg-white rounded-2xl border border-gray-200 p-8 hover:border-gray-300 hover:shadow-md transition-all">
      ...
      <CTASection_1.default title="一个软件，所有行业" description="下载基础桌面端，安装时选择行业即可自动下载对应插件。免费使用，无需承诺。" primaryButtonText="免费下载桌面端" primaryButtonLink="/download"/>

      <Footer_1.default />
    </div>);
};
exports.default = UseCasesPage;
