...
var FAQPage = function () {
    var _a = (0, react_1.useState)(null), openIndex = _a[0], setOpenIndex = _a[1];
    var toggleQuestion = function (catIdx, qIdx) {
        var key = "".concat(catIdx, "-").concat(qIdx);
        setOpenIndex(openIndex === key ? null : key);
    };
    var totalQuestions = faqData.reduce(function (sum, cat) { return sum + cat.questions.length; }, 0);
    return (<div className="min-h-screen bg-gray-50 flex flex-col">
      <RouteSEO_1.default routeKey="faq"/>
      <Navbar_1.default />

      <PageHeader_1.default title="常见问题" description={"\u5171 ".concat(totalQuestions, " \u4E2A\u95EE\u9898\uFF0C\u5E2E\u4F60\u5FEB\u901F\u4E86\u89E3 ProClaw")}/>

      {/* FAQ Content */}
      <div className="flex-grow py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          {faqData.map(function (category, catIdx) { return (<section key={catIdx} className="mb-12">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 pb-2 border-b border-gray-200">
                {category.name}
              </h2>
              <div className="space-y-3">
                {category.questions.map(function (faq, qIdx) {
                var key = "".concat(catIdx, "-").concat(qIdx);
                var isOpen = openIndex === key;
                return (<div key={qIdx} className={"bg-white rounded-xl border transition-all cursor-pointer ".concat(isOpen ? 'border-black shadow-md' : 'border-gray-200 hover:border-gray-300')} onClick={function () { return toggleQuestion(catIdx, qIdx); }}>
                      <div className="p-5 flex items-start justify-between gap-4">
                        <div className="flex items-start gap-3 min-w-0">
                          {faq.featured && (<svg className="w-5 h-5 text-yellow-500 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>)}
                          <div>
                            <h3 className="text-lg font-semibold text-gray-900 pr-8">{faq.question}</h3>
                            {isOpen && (<p className="mt-3 text-gray-600 leading-relaxed whitespace-pre-line">{faq.answer}</p>)}
                          </div>
                        </div>
                        <svg className={"w-5 h-5 text-gray-400 shrink-0 mt-1 transition-transform ".concat(isOpen ? 'rotate-180' : '')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-gray-100 py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-600 mb-4">
            前往{' '}
            <a href="https://github.com/BigLionX/ProClaw/issues" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
              GitHub Issues
            </a>{' '}
