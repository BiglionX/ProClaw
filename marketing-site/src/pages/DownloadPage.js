...
var TokenCostEstimator_1 = require("../components/TokenCostEstimator");
...
var productCards = [
    {
        ...
        name: 'ProClaw Plus',
        subtitle: '进销存版',
        icon: (<svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
...
var DownloadPage = function () {
    ...
    var _b = (0, react_1.useState)(true), pluginsLoading = _b[0], setPluginsLoading = _b[1];
    ...
    return (<div className="min-h-screen bg-gray-50 flex flex-col">
      <RouteSEO_1.default routeKey="download"/>
      <Navbar_1.default />

      <PageHeader_1.default title="下载 ProClaw" description="ProClaw Plus / Light 桌面端 · Cloud 云商城 · 手机端 App" subtitle="Plus 版 v1.0.0-beta.2 | Light 版 v1.0.0-beta.2 | 发布日期：2026-05-31"/>
      ...
      <div className="flex-grow py-12">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-4 gap-5 mb-16">
            {productCards.map(function (card) { return (<div key={card.id} className={"rounded-2xl p-6 border-2 relative flex flex-col ".concat(card.highlighted
                ? 'border-black bg-white ring-4 ring-gray-100'
                : 'border-gray-200 bg-white hover:border-gray-400 hover:shadow-md transition-all')}>
                {card.highlighted && (<span className="absolute -top-3 left-4 px-3 py-0.5 bg-black text-white text-xs font-semibold rounded-full">
                    推荐
                  </span>)}

                {/* 图标 + 标题 */}
                <div className="text-center mb-4">
                  <div className={"w-12 h-12 rounded-xl flex items-center justify-center mx-auto mb-3 ".concat(card.color === 'blue' ? 'bg-blue-600 text-white' : 'bg-gray-900 text-white')}>
                    {card.icon}
                  </div>
                ...
                <ul className="space-y-2 mb-6 flex-grow">
                  {card.features.map(function (f) { return (<li key={f} className="flex items-start gap-2 text-xs text-gray-600">
                ...
                {/* CTA */}
                {card.ctaLink.startsWith('http') ? (<a href={card.ctaLink} target="_blank" rel="noopener noreferrer" className={"block w-full text-center py-2.5 rounded-lg text-sm font-medium transition-all ".concat(card.color === 'blue'
                    ? 'bg-blue-600 text-white hover:bg-blue-700'
                    : 'bg-black text-white hover:bg-gray-800')}>
                    {card.cta}
                  </a>) : (<react_router_dom_1.Link to={card.ctaLink} className={"block w-full text-center py-2.5 rounded-lg text-sm font-medium transition-all ".concat(card.color === 'blue'
                    ? 'bg-blue-600 text-white hover:bg-blue-700'
                    : 'bg-black text-white hover:bg-gray-800')}>
                    {card.cta}
          ...
          <div className="mb-16">
            ...
            {pluginsLoading ? (<div className="text-center py-8 text-gray-400">加载中...</div>) : (<div className="grid md:grid-cols-4 gap-4">
                {plugins.map(function (plugin) { return (<div key={plugin.id} className="bg-white rounded-xl border border-gray-200 p-6 text-center hover:border-gray-300 hover:shadow-md transition-all">
                ...
                {/* 预留位 */}
                {[{ id: 'beauty', name: '美业版', icon: '💇' }, { id: 'pet', name: '宠物版', icon: '🐾' }].map(function (placeholder) {
                if (plugins.find(function (p) { return p.id === placeholder.id; }))
                    return null;
                return (<div key={placeholder.id} className="bg-white rounded-xl border border-gray-200 p-6 text-center opacity-60">
          ...
          <div id="pricing" className="mb-16 scroll-mt-20">
            ...
            <div className="grid md:grid-cols-2 gap-8 mb-16">
              {desktopPlans.map(function (plan) { return (<div key={plan.name} className={"rounded-2xl p-8 border-2 ".concat(plan.highlighted
                ? 'border-black bg-white ring-4 ring-gray-100'
                : 'border-gray-200 bg-white')}>
                  {plan.highlighted && (<span className="inline-block px-3 py-1 bg-black text-white text-xs font-semibold rounded-full mb-4">
                      推荐
                    </span>)}
                  ...
                  <ul className="space-y-3 mb-8">
                    {plan.features.map(function (f) { return (<li key={f} className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <react_router_dom_1.Link to={plan.ctaLink} className={"block w-full text-center py-3 rounded-lg font-medium transition-all ".concat(plan.highlighted
                ? 'bg-black text-white hover:bg-gray-800'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              {cloudTokenPricing.pricing.map(function (group) { return (<div key={group.category} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  <div className="bg-gray-50 px-5 py-3 border-b border-gray-200">
                    <h4 className="font-semibold text-gray-900 text-sm">{group.category}</h4>
                  </div>
                  <div className="divide-y divide-gray-100">
                    {group.items.map(function (item) { return (<div key={item.action} className="px-5 py-3 flex items-center justify-between">
                        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-12">
              {cloudTokenPricing.packages.map(function (pkg) { return (<div key={pkg.name} className="bg-white border border-gray-200 rounded-xl p-5 text-center hover:border-gray-400 transition-colors">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 mt-8 mb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {cloudTokenPricing.freeAllowance.map(function (item) { return (<div key={item.item} className="bg-gray-50 rounded-xl p-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8">
              ...
              <div className="space-y-6">
                {pricingFaqs.map(function (faq, i) { return (<div key={i} className="border-b border-gray-200 pb-6 last:border-b-0 last:pb-0">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8 mb-8">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody>
                  {systemRequirements.map(function (req) { return (<tr key={req.item} className="border-b border-gray-100">
          ...
          <div className="grid md:grid-cols-1 gap-6 max-w-sm mx-auto">
            <a href="https://github.com/BigLionX/ProClaw" target="_blank" rel="noopener noreferrer" className="bg-white rounded-xl border border-gray-200 p-6 hover:border-black hover:shadow-md transition-all text-center">
