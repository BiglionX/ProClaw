...
var ChangelogPage = function () {
    return (<div className="min-h-screen bg-gray-50 flex flex-col">
      <RouteSEO_1.default routeKey="changelog"/>
      <Navbar_1.default />

      <PageHeader_1.default title="发布日志" description="跟踪 ProClaw 的每个版本更新与改进。"/>

      {/* Release List */}
      <div className="flex-grow py-12">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          {releases.map(function (release) { return (<div key={release.version} className="mb-12">
              {/* Version Header */}
              ...
              <div className="space-y-6">
                {release.sections.map(function (section) { return (<div key={section.type}>
                    <span className={"inline-block px-3 py-1 rounded-full text-xs font-semibold mb-3 ".concat(section.color)}>
                      {section.label}
                    </span>
                    <ul className="space-y-2">
                      {section.items.map(function (item, i) { return (<li key={i} className="flex items-start gap-2 text-sm text-gray-600">
          ...
          <p className="text-center text-gray-400 text-sm mt-4">
            完整更新历史请查看{' '}
            <a href="https://github.com/BigLionX/ProClaw/blob/main/CHANGELOG.md" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
