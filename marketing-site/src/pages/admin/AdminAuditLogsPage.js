...
var AdminAuditLogsPage = function () {
    var navigate = (0, react_router_dom_1.useNavigate)();
    var _a = (0, react_1.useState)(''), searchTerm = _a[0], setSearchTerm = _a[1];
    var _b = (0, react_1.useState)('all'), actionFilter = _b[0], setActionFilter = _b[1];
    var _c = (0, react_1.useState)('all'), statusFilter = _c[0], setStatusFilter = _c[1];
    var _d = (0, react_1.useState)('7d'), dateRange = _d[0], setDateRange = _d[1];
    var _e = (0, react_1.useState)(null), selectedLog = _e[0], setSelectedLog = _e[1];
    // 模拟审计日志数据
    var logs = (0, react_1.useState)([
        {
            id: 'log-001',
            timestamp: '2024-04-14 15:30:22',
            userId: 'admin-001',
            userEmail: 'admin@proclaw.cc',
            action: '更新系统设置',
    ...
    var filteredLogs = logs.filter(function (log) {
        ...
        var matchesAction = actionFilter === 'all' || log.method === actionFilter;
    ...
    var getMethodBadge = function (method) {
        ...
        return (<span className={"px-2 py-1 text-xs font-medium rounded-full ".concat(styles[method])}>
        {method}
      </span>);
    };
    var getStatusBadge = function (status) {
        return status === 'success' ? (<span className="inline-flex items-center text-green-600">
    ...
    var successLogs = logs.filter(function (l) { return l.status === 'success'; }).length;
    var failedLogs = logs.filter(function (l) { return l.status === 'failed'; }).length;
    return (<div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <exportUtils_1.ExportButton data={filteredLogs} filename="audit_logs" headers={[
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              ...
              <input type="text" value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} placeholder="搜索操作、用户或资源..." className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none"/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">操作类型</label>
              <select value={actionFilter} onChange={function (e) { return setActionFilter(e.target.value); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLogs.map(function (log) { return (<tr key={log.id} className="hover:bg-gray-50 cursor-pointer" onClick={function () { return setSelectedLog(log); }}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {log.resourceId && (<div className="text-xs text-gray-500 font-mono">{log.resourceId}</div>)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
      ...
      {/* Log Detail Modal */}
      {selectedLog && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-900">日志详情</h2>
                <button onClick={function () { return setSelectedLog(null); }} className="text-gray-400 hover:text-gray-600">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
                  </svg>
                </button>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {selectedLog.resourceId && (<div className="text-xs text-gray-500 font-mono mt-1">{selectedLog.resourceId}</div>)}
                </div>
              ...
              <div className="mt-6 flex justify-end">
                <button onClick={function () { return setSelectedLog(null); }} className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors">
                  关闭
                </button>
              </div>
            </div>
...
exports.default = AdminAuditLogsPage;
