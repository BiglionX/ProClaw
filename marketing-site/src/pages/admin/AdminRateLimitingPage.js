...
var AdminRateLimitingPage = function () {
    var navigate = (0, react_router_dom_1.useNavigate)();
    var _a = (0, react_1.useState)(false), showAddModal = _a[0], setShowAddModal = _a[1];
    var _b = (0, react_1.useState)('all'), scopeFilter = _b[0], setScopeFilter = _b[1];
    // 模拟限流规则数据
    var rules = (0, react_1.useState)([
        {
            id: 'rule-001',
            name: '全局默认限流',
    ...
    var filteredRules = rules.filter(function (rule) {
        return scopeFilter === 'all' || rule.scope === scopeFilter;
    });
    var getScopeBadge = function (scope) {
        var styles = {
        ...
        return (<span className={"px-2 py-1 text-xs font-medium rounded-full ".concat(styles[scope])}>
        {labels[scope]}
      </span>);
    };
    var handleToggleRule = function (id) {
        console.log("\u5207\u6362\u89C4\u5219\u72B6\u6001: ".concat(id));
    };
    var handleEditRule = function (id) {
        console.log("\u7F16\u8F91\u89C4\u5219: ".concat(id));
    };
    var handleDeleteRule = function (id) {
        if (confirm('确定要删除此限流规则吗？')) {
            console.log("\u5220\u9664\u89C4\u5219: ".concat(id));
        }
    };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <p className="text-2xl font-bold text-purple-600 mt-2">
              {rules.filter(function (r) { return r.scope === 'global'; }).length}
            </p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-6">
            <p className="text-sm text-gray-600">端点规则</p>
            <p className="text-2xl font-bold text-orange-600 mt-2">
              {rules.filter(function (r) { return r.scope === 'endpoint'; }).length}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
          <div className="flex items-center space-x-4">
            ...
            <select value={scopeFilter} onChange={function (e) { return setScopeFilter(e.target.value); }} className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none">
        ...
        <div className="space-y-4">
          {filteredRules.map(function (rule) { return (<div key={rule.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <h3 className="text-lg font-semibold text-gray-900">{rule.name}</h3>
                    {getScopeBadge(rule.scope)}
                    <span className={"px-2 py-1 text-xs font-medium rounded-full ".concat(rule.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700')}>
                      {rule.isActive ? '启用' : '禁用'}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600">{rule.description}</p>
                  {rule.endpoint && (<div className="mt-2 inline-block px-3 py-1 bg-gray-100 rounded text-sm font-mono text-gray-700">
                      {rule.endpoint}
                    </div>)}
                </div>
              </div>
              ...
              {rule.whitelist.length > 0 && (<div className="mb-4">
                  <div className="text-xs text-gray-500 mb-2">白名单 ({rule.whitelist.length})</div>
                  <div className="flex flex-wrap gap-2">
                    {rule.whitelist.map(function (item, index) { return (<span key={index} className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded">
                        {item}
                      </span>); })}
                  </div>
                </div>)}
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                <div className="flex space-x-2">
                  <button onClick={function () { return handleEditRule(rule.id); }} className="px-3 py-1.5 text-sm bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors">
                    编辑
                  </button>
                  <button onClick={function () { return handleToggleRule(rule.id); }} className={"px-3 py-1.5 text-sm rounded-lg transition-colors ".concat(rule.isActive
                ? 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100'
                : 'bg-green-50 text-green-700 hover:bg-green-100')}>
                    {rule.isActive ? '禁用' : '启用'}
                  </button>
                  <button onClick={function () { return handleDeleteRule(rule.id); }} className="px-3 py-1.5 text-sm bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition-colors">
                    删除
                  </button>
                </div>
              </div>
...
exports.default = AdminRateLimitingPage;
