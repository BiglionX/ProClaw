...
var AdminAnalyticsPage = function () {
    var navigate = (0, react_router_dom_1.useNavigate)();
    var _a = (0, react_1.useState)('7d'), timeRange = _a[0], setTimeRange = _a[1];
    // 模拟分析数据
    var analytics = (0, react_1.useState)({
        totalUsers: 1234,
        activeUsers: 856,
        newUsersToday: 23,
        ...
        apiUsageByHour: Array.from({ length: 24 }, function (_, i) { return ({
            hour: i,
    ...
    var maxApiCalls = Math.max.apply(Math, analytics.apiUsageByHour.map(function (h) { return h.calls; }));
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex space-x-2">
              <button onClick={function () { return setTimeRange('7d'); }} className={"px-4 py-2 rounded-lg text-sm font-medium transition-colors ".concat(timeRange === '7d' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 hover:bg-gray-100')}>
                近7天
              </button>
              <button onClick={function () { return setTimeRange('30d'); }} className={"px-4 py-2 rounded-lg text-sm font-medium transition-colors ".concat(timeRange === '30d' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 hover:bg-gray-100')}>
                近30天
              </button>
              <button onClick={function () { return setTimeRange('90d'); }} className={"px-4 py-2 rounded-lg text-sm font-medium transition-colors ".concat(timeRange === '90d' ? 'bg-gray-900 text-white' : 'bg-white text-gray-700 hover:bg-gray-100')}>
                近90天
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              {analytics.userGrowth.map(function (day, index) { return (<div key={index} className="flex items-center">
                  <div className="w-20 text-sm text-gray-600">{day.date.slice(5)}</div>
                  <div className="flex-1 mx-3">
                    <div className="bg-gray-200 rounded-full h-2">
                      <div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: "".concat((day.users / 30) * 100, "%") }}></div>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-2">
              {analytics.apiUsageByHour.map(function (hourData) { return (<div key={hourData.hour} className="flex items-center">
                  <div className="w-12 text-xs text-gray-600">{hourData.hour.toString().padStart(2, '0')}:00</div>
                  <div className="flex-1 mx-3">
                    <div className="bg-gray-200 rounded-full h-1.5">
                      <div className="bg-purple-600 h-1.5 rounded-full transition-all" style={{ width: "".concat((hourData.calls / maxApiCalls) * 100, "%") }}></div>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              {analytics.topEndpoints.map(function (endpoint, index) { return (<div key={index} className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="bg-gray-200 rounded-full h-2">
                      <div className="bg-indigo-600 h-2 rounded-full transition-all" style={{ width: "".concat(endpoint.percentage, "%") }}></div>
                    </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              {analytics.tokenUsageByUser.map(function (user, index) { return (<div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center">
