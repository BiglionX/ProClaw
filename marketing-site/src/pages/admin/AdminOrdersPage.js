      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <exportUtils_1.ExportButton data={filteredOrders} filename="orders" headers={[
            { key: 'id', label: '订单号' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <p className="text-2xl font-bold text-blue-600 mt-2">
              {orders.length > 0 ? Math.round((orders.filter(function (o) { return o.status === 'completed'; }).length / orders.length) * 100) : 0}%
            </p>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.map(function (order) { return (<tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {order.completedAt && (<div className="text-xs text-gray-500">完成: {order.completedAt}</div>)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button className="text-gray-900 hover:text-gray-700 mr-3">详情</button>
                      {order.status === 'pending' && (<button className="text-red-600 hover:text-red-800">取消</button>)}
                    </td>
                  </tr>); })}
              </tbody>
            </table>
