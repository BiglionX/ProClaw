...
var AdminReportsPage = function () {
    ...
    var _a = (0, react_1.useState)(''), selectedTemplate = _a[0], setSelectedTemplate = _a[1];
    var _b = (0, react_1.useState)('30d'), dateRange = _b[0], setDateRange = _b[1];
    var _c = (0, react_1.useState)(''), customDateStart = _c[0], setCustomDateStart = _c[1];
    var _d = (0, react_1.useState)(''), customDateEnd = _d[0], setCustomDateEnd = _d[1];
    var _e = (0, react_1.useState)('day'), groupBy = _e[0], setGroupBy = _e[1];
    var _f = (0, react_1.useState)(true), includeCharts = _f[0], setIncludeCharts = _f[1];
    var _g = (0, react_1.useState)(false), isGenerating = _g[0], setIsGenerating = _g[1];
    // 模拟报表模板
    var templates = (0, react_1.useState)([
        {
            id: 'template-1',
            name: '用户增长报表',
    ...
    // 模拟报表数据
    var reportData = (0, react_1.useState)({
        summary: {
            totalRecords: 1234,
    ...
    var getTemplateColor = function (type) {
        var colors = {
    ...
    var maxValue = Math.max.apply(Math, reportData.chartData.map(function (d) { return d.value; }));
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex space-x-3">
              <exportUtils_1.ExportButton data={reportData.tableData} filename={"report_".concat(selectedTemplate || 'custom')} headers={[
            { key: 'metric', label: '指标' },
            { key: 'value', label: '数值' },
            { key: 'change', label: '变化' },
            { key: 'trend', label: '趋势' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-3">
                {templates.map(function (template) { return (<button key={template.id} onClick={function () { return setSelectedTemplate(template.id); }} className={"w-full text-left p-4 rounded-lg border-2 transition-all ".concat(selectedTemplate === template.id
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300')}>
                    <div className="flex items-start justify-between mb-2">
                      <span className="font-medium text-gray-900">{template.name}</span>
                      <span className={"px-2 py-1 text-xs font-medium rounded-full border ".concat(getTemplateColor(template.type))}>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select value={dateRange} onChange={function (e) { return setDateRange(e.target.value); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none">
                ...
                {dateRange === 'custom' && (<div className="space-y-3">
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">开始日期</label>
                      <input type="date" value={customDateStart} onChange={function (e) { return setCustomDateStart(e.target.value); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none"/>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">结束日期</label>
                      <input type="date" value={customDateEnd} onChange={function (e) { return setCustomDateEnd(e.target.value); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none"/>
                    </div>
                  </div>)}

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">分组方式</label>
                  <select value={groupBy} onChange={function (e) { return setGroupBy(e.target.value); }} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none">
                ...
                <div className="flex items-center justify-between py-2">
                  ...
                  <button onClick={function () { return setIncludeCharts(!includeCharts); }} className={"relative inline-flex h-6 w-11 items-center rounded-full transition-colors ".concat(includeCharts ? 'bg-blue-600' : 'bg-gray-200')}>
                    <span className={"inline-block h-4 w-4 transform rounded-full bg-white transition-transform ".concat(includeCharts ? 'translate-x-6' : 'translate-x-1')}/>
                  </button>
                </div>

                <button onClick={handleGenerateReport} disabled={isGenerating || !selectedTemplate} className="w-full px-4 py-3 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              {/* Chart Visualization */}
              {includeCharts && (<div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">趋势图</h4>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="space-y-2">
                      {reportData.chartData.map(function (data, index) { return (<div key={index} className="flex items-center">
                          <div className="w-20 text-xs text-gray-600">{data.date.slice(5)}</div>
                          <div className="flex-1 mx-3">
                            <div className="bg-gray-200 rounded-full h-3">
                              <div className={"h-3 rounded-full transition-all ".concat(data.growth >= 0 ? 'bg-green-500' : 'bg-red-500')} style={{ width: "".concat((data.value / maxValue) * 100, "%") }}></div>
                            </div>
                          </div>
                          <div className="w-24 flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-900">{data.value}</span>
                            <span className={"text-xs ".concat(data.growth >= 0 ? 'text-green-600' : 'text-red-600')}>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {reportData.tableData.map(function (row, index) { return (<tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className={"px-4 py-3 text-sm font-medium ".concat(row.change.startsWith('+') ? 'text-green-600' : 'text-red-600')}>
                            {row.change}
                          </td>
                          <td className="px-4 py-3">
                            {row.trend === 'up' ? (<span className="inline-flex items-center text-green-600">
                                <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18"/>
                                </svg>
                                上升
                              </span>) : (<span className="inline-flex items-center text-red-600">
                                <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
