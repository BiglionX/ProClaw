...
const TokenCostEstimator: React.FC = () => {
  const [usage, setUsage] = useState<Record<string, number>>(
    Object.fromEntries(USAGE_INPUTS.map(i => [i.key, i.defaultValue]))
  );

  const updateUsage = (key: string, value: string) => {
    const num = parseInt(value) || 0;
    setUsage(prev => ({ ...prev, [key]: Math.max(0, num) }));
  };

  // 计算总消耗
  const totals = useMemo(() => {
    let totalPt = 0;
    const details = USAGE_INPUTS.map(input => {
      const qty = usage[input.key] || 0;
      const pt = input.ptCost * qty;
      totalPt += pt;
      return { ...input, quantity: qty, pt, cost: pt * 0.001 };
    });
    return { totalPt, totalCost: totalPt * 0.001, details };
  }, [usage]);

  // 推荐套餐
  const recommendedPackage = useMemo(() => {
    // 建议充值量为月消耗的 1.5~2 倍
    const suggested = totals.totalPt * 1.5;
    let best = RECHARGE_PACKAGES[0];
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        {USAGE_INPUTS.map(input => (
          <div key={input.key} className="flex items-center justify-between bg-gray-50 rounded-lg px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={e => updateUsage(input.key, e.target.value)}
                min="0"
                className="w-20 px-2 py-1.5 border border-gray-300 rounded-lg text-sm text-right outline-none focus:ring-2 focus:ring-gray-900"
              />
      ...
      <details className="mb-6">
        ...
        <div className="mt-4 space-y-2">
          {totals.details.map(d => (
            <div key={d.key} className="flex items-center justify-between px-4 py-2 bg-gray-50 rounded-lg text-sm">
