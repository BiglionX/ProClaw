...
-- 添加新字段到旧表(用于兼容性)
ALTER TABLE products ADD COLUMN IF NOT EXISTS spu_id TEXT REFERENCES product_spus(id);
ALTER TABLE products ADD COLUMN IF NOT EXISTS sku_id TEXT REFERENCES product_skus(id);

-- ============================================
-- 触发器: 自动更新updated_at时间戳
-- ============================================
-- 注意: PostgreSQL的DROP TRIGGER需要指定表名
DO $$ 
BEGIN
    -- 如果触发器存在则删除
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_product_spus_updated_at') THEN
        DROP TRIGGER update_product_spus_updated_at ON product_spus;
    END IF;
END $$;
...
BEGIN
    -- 如果触发器存在则删除
    IF EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_product_skus_updated_at') THEN
        DROP TRIGGER update_product_skus_updated_at ON product_skus;
    END IF;
END $$;
...
-- 创建视图: SPU库存汇总 (方便查询)
-- ============================================
-- 注意: PostgreSQL中DROP VIEW IF EXISTS需要CASCADE以防依赖
DROP VIEW IF EXISTS v_spu_inventory CASCADE;
...
-- ============================================
-- 注意: PostgreSQL中DROP VIEW IF EXISTS需要CASCADE以防依赖
DROP VIEW IF EXISTS v_spu_details CASCADE;
