...
pub async fn list_users(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({ "data": users, "total": users.len() })),
    )
}
    ...
    if let Some(ref role_name) = payload.role {
        if let Err(e) = assign_user_role(&conn, &id, role_name) {
            if let Err(db_err) = conn.execute("DELETE FROM users WHERE id = ?1", params![id]) {
                eprintln!(
                    "[Users] Failed to rollback user after role assignment error: {}",
                    db_err
                );
            }
            return (
...
pub async fn update_user(
    ...
    if let Some(ref phone) = payload.phone {
        p.push(Box::new(phone.clone()));
    }
    if let Some(ref email) = payload.email {
        p.push(Box::new(email.clone()));
    }
    if let Some(is_active) = payload.is_active {
        p.push(Box::new(is_active));
    }
    p.push(Box::new(id));
...
pub async fn delete_user(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!({"message": "User deactivated successfully"})),
    )
}
...
pub async fn change_password(
    ...
    if let Some(ref old_pwd) = payload.old_password {
        ...
        if !super::auth::verify_password(old_pwd, current_hash.as_deref()) {
            return (
                StatusCode::BAD_REQUEST,
                Json(serde_json::json!({"error": "Old password is incorrect"})),
            );
        }
    }
...
fn assign_user_role(
    ...
    if let Err(e) = conn.execute(
        ...
        eprintln!(
            "[Users] Failed to clear existing roles for {}: {}",
            user_id, e
        );
    }
