...
pub fn generate_statement(
    ...
    // 查询对方信息
    let (counterparty_name, _email): (Option<String>, Option<String>) =
    ...
    let transactions: Vec<serde_json::Value> = stmt
        .query_map(
            params![counterparty_id, counterparty_type, start_date, end_date],
            |row| {
                Ok(serde_json::json!({
...
fn generate_pdf(
    ...
    let label: &str = if counterparty_type == "supplier" {
        "供应商"
    } else {
        "客户"
    };
    ...
    match format {
        ...
        "open_items" => {
            ...
            for txn in transactions {
                ...
                let amount = txn["amount"].as_f64().unwrap_or(0.0);
                total_unpaid += if txn["txn_type"].as_str().unwrap_or("") == "receipt" {
                    -amount
                } else {
                    amount
                };
        ...
        _ => {
            ...
            for txn in transactions {
                ...
                let debit_str = if is_debit {
                    format!("¥{:.2}", amount)
                } else {
                    String::new()
                };
                let credit_str = if !is_debit {
                    format!("¥{:.2}", amount)
                } else {
                    String::new()
                };
                ...
                let line = format!(
                    "{}  {:<6}  {:>10}  {:>10}  ¥{:.2}",
                    date, txn_type, debit_str, credit_str, running_balance
                );
...
pub fn send_statement_email(
    ...
    let smtp_port: u16 = get_config("smtp_port")?
        .parse()
...
pub fn create_reconciliation_rule(
    ...
    let statement_format = rule["statement_format"]
        .as_str()
        .ok_or("对账单格式不能为空")?;
...
pub fn delete_reconciliation_rule(
    ...
    conn.execute(
        "DELETE FROM reconciliation_rules WHERE id = ?1",
        params![rule_id],
    )
    .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn get_smtp_config(db: tauri::State<Mutex<Database>>) -> Result<serde_json::Value, String> {
    let db = db.lock().map_err(|e| e.to_string())?;
    let conn = db.connection();
        ...
        let should_trigger = match trigger_type.as_str() {
            "date" => {
                ...
                if let Ok(cfg) = serde_json::from_str::<serde_json::Value>(trigger_config) {
                    ...
                    let today_day = chrono::Local::now()
                        .format("%d")
                        .to_string()
                        .parse::<i64>()
                        .unwrap_or(1);
