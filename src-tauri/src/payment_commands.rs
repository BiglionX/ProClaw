...
pub fn record_payment_cmd(
    ...
    // 查询采购订单信息
    let (total_amount, supplier_id, supplier_name): (f64, String, Option<String>) = conn
        .query_row(
    ...
    let payment_status =
        if (new_paid - total_amount).abs() < f64::EPSILON || new_paid >= total_amount {
            "paid"
        } else {
            "partial"
        };
...
pub fn record_receipt_cmd(
    ...
    // 查询销售订单信息
    let (total_amount, customer_id, customer_name): (f64, String, Option<String>) = conn
        .query_row(
