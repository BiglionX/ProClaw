...
pub fn ap_search_by_oe(
    db: tauri::State<Mutex<Database>>, oe_number: String,
) -> Result<Value, String> {
...
pub fn ap_add_vehicle_model(
    db: tauri::State<Mutex<Database>>, brand: String, series: Option<String>, year_range: Option<String>, displacement: Option<String>,
) -> Result<Value, String> {
...
pub fn ap_add_oe_number(
    db: tauri::State<Mutex<Database>>, product_id: String, oe_number: String,
    vehicle_model_id: Option<String>, part_category: Option<String>,
) -> Result<Value, String> {
    let db = db.lock().map_err(|e| e.to_string())?; let conn = db.connection(); let id = Uuid::new_v4().to_string();
