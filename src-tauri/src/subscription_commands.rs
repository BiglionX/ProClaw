...
/// 获取当前用户订阅
#[tauri::command]
pub fn get_my_subscription_cmd(
...
/// 取消订阅
#[tauri::command]
pub fn cancel_subscription_cmd(
    db: State<Mutex<Database>>,
    user_id: String,
) -> Result<serde_json::Value, String> {
    let db = db.lock().map_err(|e| e.to_string())?;
...
pub fn get_token_usage_cmd(
    ...
    let db = db.lock().map_err(|e| e.to_string())?;
    let details = subscription_service::get_token_usage_details(
        &db,
        &user_id,
        limit.unwrap_or(50),
        offset.unwrap_or(0),
...
pub fn record_token_cmd(
    ...
    let db = db.lock().map_err(|e| e.to_string())?;
    subscription_service::record_token_usage(
        &db,
        &user_id,
        &action_type,
...
#[tauri::command]
pub fn estimate_token_cost_cmd(
