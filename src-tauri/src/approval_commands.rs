...
pub fn record_payment_cmd(
    ...
    if !["purchase_order", "sales_order"].contains(&order_type.as_str()) {
    ...
    conn.execute(
        "INSERT INTO payments (id, order_type, order_id, amount, payment_method, payment_date, notes) \
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        params![id, order_type, order_id, amount, method, today, notes],
    ).map_err(|e| e.to_string())?;

    // 更新订单已付金额
    let table = if order_type == "purchase_order" { "purchase_orders" } else { "sales_orders" };
    let paid_field = if order_type == "purchase_order" { "paid_amount" } else { "paid_amount" };

    let _ = conn.execute(
        &format!("UPDATE {} SET {} = {} + ?1, updated_at = CURRENT_TIMESTAMP WHERE id = ?2", table, paid_field, paid_field),
        params![amount, order_id],
    );

    // 检查是否付清
    let (total, paid): (f64, f64) = conn.query_row(
        &format!("SELECT total_amount, {} FROM {} WHERE id = ?1", paid_field, table),
        params![order_id],
    ...
    if paid >= total {
        let _ = conn.execute(
            &format!("UPDATE {} SET payment_status = 'paid', updated_at = CURRENT_TIMESTAMP WHERE id = ?1", table),
            params![order_id],
        );
    } else if paid > 0.0 {
        let _ = conn.execute(
            &format!("UPDATE {} SET payment_status = 'partial', updated_at = CURRENT_TIMESTAMP WHERE id = ?1", table),
            params![order_id],
        );
    }
    ...
    Ok(serde_json::json!({
        ...
        "paid_amount": paid,
        "payment_status": if paid >= total { "paid" } else if paid > 0.0 { "partial" } else { "unpaid" },
    }))
}
...
pub fn get_payments_cmd(
    ...
    let mut sql = String::from(
        "SELECT id, order_type, order_id, amount, payment_method, payment_date, notes, created_at FROM payments WHERE 1=1"
    );
    let mut pv: Vec<Box<dyn rusqlite::types::ToSql>> = Vec::new();

    if let Some(ref t) = order_type {
        sql.push_str(&format!(" AND order_type = ?{}", pv.len() + 1));
        pv.push(Box::new(t.clone()));
    }
    if let Some(ref oid) = order_id {
        sql.push_str(&format!(" AND order_id = ?{}", pv.len() + 1));
        pv.push(Box::new(oid.clone()));
    }
    ...
    let rows = stmt.query_map(refs.as_slice(), |row| {
        Ok(serde_json::json!({
            ...
            "payment_date": row.get::<_, String>(5)?,
            "notes": row.get::<_, Option<String>>(6)?,
            "created_at": row.get::<_, String>(7)?,
        }))
    }).map_err(|e| e.to_string())?;

    let payments: Vec<serde_json::Value> = rows.filter_map(|r| r.ok()).collect();
    Ok(serde_json::json!({ "data": payments, "total": payments.len() }))
}
