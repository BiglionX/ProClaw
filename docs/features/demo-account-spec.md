...
|------|-----|
| 子域名 | `demo` |
| 访问 URL | `https://proclaw.cc/shop/demo`（路径模式，非子域名模式） |
| 套餐类型 | `free` |
| 状态 | `active` |
...
| 云商城 | `demoFlag.cloudStoreSubdomain` | `已开通云商城（proclaw.cc/shop/demo）` |
...
/** 演示云商城访问 URL */
const DEMO_CLOUD_STORE_URL = 'https://proclaw.cc/shop/demo';
...
| **云商城 URL 模式** | 使用路径模式（`proclaw.cc/shop/{商店名}`）而非子域名模式（`demo.proclaw.cc`） |
