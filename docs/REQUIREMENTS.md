...
> **下一里程碑**: v1.1.0（v13.0 离线优先模式草案已立项）
...
> **v13.0 离线优先（草案）**：自 v1.1.0 起，**ProClaw Plus / Light 双模式均支持离线启动**。用户首次安装后无需登录即可使用商品/销售/库存/客户/媒体库/AI 助手（本地回退）等全功能；登录仅作为云端增值能力凭证（云备份、AI Team 群聊、Token 计费、多端同步），入口收缩至「侧边栏底部账号区 + 设置中心账号 Tab」。详见 [桌面端离线优先与本地账号体系 PRD v13.0](prd/architecture/需求文档：ProClaw%20桌面端离线优先与本地账号体系（PRD%20v13.0）.md)。
...
| 🔵 部分实现 | 6 | 核心 ≥ 60%，有待补项 |
| 🟡 草案 | 3 | 插件系统 v10.0、桌面端商品数据导入 v1.0、桌面端离线优先 v13.0 |
| 🔴 已替代 | 1 | PRD v4.0 |
