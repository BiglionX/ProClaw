...
**核心功能重点**：
1. **用户管理** - 注册、登录、权限控制、多租户支持
2. **大模型调用** - AI智能体、自然语言交互、Dify集成
3. **Token管理** - API密钥管理、用量统计、成本控制
4. **外部资源桥接** - 连接各种API、Webhook、第三方服务
5. **技能生态** - 可扩展的插件系统

**部署模式**：
- **本地优先（默认）**：数据存储在本地SQLite，完全自主可控，免费开源
- **云托管（可选）**：服务端提供数据存储和同步，付费订阅
