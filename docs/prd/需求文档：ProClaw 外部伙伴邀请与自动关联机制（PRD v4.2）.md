...
| **关联发布** | [RELEASE_NOTES_v1.0.0.md](../../RELEASE_NOTES_v1.0.0.md) §"安全与性能 - 邀请码防伪（HMAC-SHA256）" |
| **覆盖率** | ~90%（邀请码生成/扫码/自动关联主流程已上线；过期/重发等边界完善） |
| **代码入口** | `src/services/invitation/`、`src/pages/pages/InvitationManagementPage.tsx`、`src-tauri/src/commands/invitation.rs` |
| **数据库依赖** | `database/complete_schema.sql`（invitations + external_partners） |
| **测试覆盖** | `e2e/invitation.spec.ts` |
| **差异与遗留** | 外部伙伴邀请与自动关联已上线；多端协同与跨店铺邀请待 v1.x |
| **后续动作** | 维持现状；按市场反馈完善 |
...
| 日期 | 状态 | 变更人/触发事件 |
|---|---|---|
| 2026-06-08 | ✅ 已实现 v1.0+ | v1.0.0 发布，外部伙伴邀请机制上线 |
