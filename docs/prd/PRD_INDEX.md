...
| 31 | 云商城 | ProClaw 云托管商城（AI 生成独立站）PRD v5.0 | ✅ |
| 32 | 云商城 | ProClaw 云托管商城 Token 计费模式改造（PRD v8.0） | ✅ |
| 33 | 云商城 | 云商城开通引导流程（PRD v6.0） | ✅ |
| 34 | 云商城 | 云商城 AI 客服模块 | 🔵 60% |
| 35 | 云商城 | 云商城客服接入 ProClaw 桌面端（方案 A） | 🔵 40% |
| 36 | Cloud | ProClaw Cloud 托管版（Web 端 + 按 token 计费） | ✅ |
| 37 | 架构 | ProClaw 架构分层优化方案（PRD v1.0） | ✅ |
| 38 | 架构 | ProClaw-Light 桌面端需求 | ✅ |
| 39 | 架构 | ProClaw 虚拟公司版（Agent 化架构）PRD v6.0 | ✅ |
| 40 | 架构 | 灵活库存需求-ProClaw 核心（PRD v12.0） | ✅ |
| 41 | 架构 | 数据中心 AI 任务概览子页面 | ✅ |
| 42 | 集成 | PROCLAW-NVWAX-API-INTEGRATION-REQUIREMENT | ✅ |
| 43 | (客服接入 #35 已计，不重复) | — | — |

**校核**：43 份纯 PRD 计数完整 = 3（CEO Agent）+ 6（移动端）+ 9（营销/用户中心）+ 12（插件/供应链/桌面端 UI/邀请/通知）+ 6（云商城/Cloud）+ 7（架构/库存/其他）= 43 ✓
