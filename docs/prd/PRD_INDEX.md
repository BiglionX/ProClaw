...
> 注：F 组按计划是 7 份；上述 6 份列出后，第 7 份为 v4.0（已归入 B 组"已替代"项）。**新增 v13.0 离线优先 PRD 见下方「G. 离线优先」分组**。

### G. 离线优先（1）

| PRD | 版本 | 状态 | 关键差异 |
|---|---|---|---|
| [ProClaw 桌面端离线优先与本地账号体系](architecture/需求文档：ProClaw%20桌面端离线优先与本地账号体系（PRD%20v13.0）.md) | v13.0 | 🟡 草案 | 离线访客可用全功能；登录收缩至设置中心「账号」Tab；本地账号 + Tauri Keyring 加密 |
...
- **ProClaw 桌面端离线优先与本地账号体系 v13.0** — 离线访客可全功能使用；登录收缩至设置中心「账号」Tab；本地账号 + Tauri Keyring；v13.1 启动实施
