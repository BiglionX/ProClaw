...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (params.search) {
      query = query.or(`po_number.ilike.%${params.search}%,notes.ilike.%${params.search}%`);
    }
    ...
    if (orderIds.length > 0) {
      const { data: items } = await supabase
        .from(schemaTable(schema, 'purchase_order_items'))
        .select('*')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabase
      .from(schemaTable(schema, 'purchase_orders'))
      .insert({
        po_number: body.po_number || `PO-${Date.now().toString(36).toUpperCase()}`,
